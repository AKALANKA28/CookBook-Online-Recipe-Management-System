...
if(isset($_POST['submit'])){
   ...
   $verify_email->store_result(); // Store the result set

   if($verify_email->num_rows > 0){
      $warning_msg[] = 'Email already taken!';
   }else{
      if($c_pass == 1){
         // Close the previous result set before executing a new query
         $verify_email->free_result();

         $insert_user = $conn->prepare("INSERT INTO `users`(id, name, email, password, image) VALUES(?,?,?,?,?)");
         $insert_user->bind_param("sssss", $id, $name, $email, $pass, $rename);
         $insert_user->execute();
